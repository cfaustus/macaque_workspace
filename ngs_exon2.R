...
setnames(keyDT,c('Sample.ID','barcode_ID'),c('subject','barcode'))
...
head(DT1)
DT1 = DT1[keyDT,on='barcode',nomatch=0]
...
# p <- ggplot(DT1[,.(count=sum(count)),by=subject],aes(count))
# p
# p+geom_histogram()
...
setkey(DT1,count)
DT1[,.(count=sum(count)),by=subject][count<1000]
# subject count
# 1:  Mf1_24   953
# 2:  Mf1_31   659
# 3:  Mf1_42   320
...
DT1 = DT1[subject!='Mf1_13']
DT1 = DT1[subject!='Mf1_25']
DT1 = DT1[subject!='Mf1_42']
DT1 = DT1[subject!='Mf1_51']


#now let's look at frequency of individual sequences
...
setkey(DT1,freq)
DT1[count>0,freq] #check by eyeballing - a 2% cut is fine (the gap is between 1.7% and 2.6%)

fDT1 <- DT1[freq>2]

#how many reads were removed?
fDT1[,sum(freq)]/DT1[,sum(freq)]
#[1] 0.9740864 #<3% removed

# frequency table 
library(tidyr)
head(fDT1)
fDT1_wide = spread(fDT1[,c(3,5,6)],uniqueseq, freq)
#write.csv(fDT1_wide, 'data/alpha1_exon1_freqtable.csv', row.names = F)
#now make plots - heatmaps and barcharts
#how many alleles are left?
...
DT2[,.(count=sum(count)),by=subject][count<1000]
# subject count
# 1:  Mf1_42   342
# 2:  Mf1_51    25

#let's eliminate the very lowest
DT2 = DT2[subject!='Mf1_51']


#now let's look at frequency of individual sequences
p <- ggplot(DT2,aes(freq))
p+geom_histogram()
p+geom_histogram() + xlim(0,5) +ylim(0,200) #nothing between ~1.9% and ~2.1%

#eliminate everything under 2%
setkey(DT2,freq)
DT2[count>0,freq] #check by eyeballing - a 2% cut is fine (the gap is between 1.7% and 2.6%)

fDT2 <- DT2[freq>2]

head(fDT2)
fDT2_wide = spread(fDT2[,c(3,5,6)],uniqueseq, freq)
write.csv(fDT2_wide, 'data/alpha2_exon1_freqtable.csv', row.names = F)

#how many reads were removed?
fDT2[,sum(freq)]/DT2[,sum(freq)]
# [1] 0.9783893#<3% removed


#now make plots - heatmaps and barcharts
#how many alleles are left?
length(fDT2[,unique(uniqueseq)])
# [1] 12

#how many subjects are left?
length(fDT2[,unique(subject)])
# [1] 77

#look at mean freqs of sequences:
fDT2[,.(freq=sum(freq)/77),by=uniqueseq]

#look at max freqs of sequences:
fDT2[,.(freq=max(freq)),by=uniqueseq]
